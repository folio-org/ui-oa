...
class SettingField extends React.Component {
  ...
  renderSettingValue = (setting) => {
    ...
    if (setting.settingType === 'Refdata') {
      ...
      return (
        <p>
          {settingLabel || (setting.defValue ? `[default] ${setting.defValue}` : <FormattedMessage id="ui-oa.settings.no-current-value" />)}
        </p>
      );
    } else if (setting.settingType !== 'Password') {
      return (
        <p>
          {setting.value ? setting.value : (setting.defValue ? `[default] ${setting.defValue}` : <FormattedMessage id="ui-oa.settings.no-current-value" />)}
        </p>
      );
    } else {
      return (
        <p>
          {setting.value ? '********' : (setting.defValue ? '[default] ********' : <FormattedMessage id="ui-oa.settings.no-current-value" />)}
        </p>
      );
    }
  }
  ...
  renderEditButton() {
    ...
    if (editing === true) {
      EditText = <FormattedMessage id="ui-oa.settings.finish-editing" />;
      return (
        <Button
          onClick={(e) => {
            e.preventDefault();
      ...
      EditText = <FormattedMessage id="ui-oa.settings.edit" />;
      return (
        <Button
          onClick={(e) => {
            e.preventDefault();
  ...
  render() {
    ...
    const id = `ui-oa.settingName.${camelKey}.help`;

    return (
      <Card
        headerEnd={this.renderEditButton()}
        headerStart={currentSetting ? <FormattedMessage id={`ui-oa.settingName.${camelKey}`} /> : <FormattedMessage id="ui-oa.settingName.settingLoading" />}
        roundedBorder
      >
        {renderFunction}
